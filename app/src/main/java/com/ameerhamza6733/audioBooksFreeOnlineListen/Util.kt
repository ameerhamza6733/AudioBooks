...
//    val gaboora_URL = "https://archive.org/advancedsearch.php?q=collection%3A%22audio_bookspoetry%22+AND+mediatype%3Aaudio&fl[]=avg_rating&fl[]=publisher&fl[]=description&fl[]=downloads&fl[]=identifier&fl[]=mediatype&fl[]=num_reviews&fl[]=title&sort[]=-downloads&sort[]=&sort[]=&rows=100&page=1&output=json&callback=callback&save=yes"
    ...
    fun isWorkFromCollectionDomain(jsonObject: JSONObject):Boolean{
        return try {
            val jsonArrayCollection =  jsonObject.getJSONArray("collection")
            val licenseurl = jsonObject.getString("licenseurl")
            if (licenseurl.contains("creativecommons")){
                for ( i  in 0 until jsonArrayCollection.length()){
                    return jsonArrayCollection.get(i).toString().lowercase().contains("librivox")
                }
            }
